...
Bei\ChangedAt{v3.44}{\Class{scrlttr2}\and \Package{scrletter}} Briefen mit
mehreren Unterzeichners können die einzelnen Signaturen innerhalb der
Variablen \Variable{signature} mit \Macro{and} voneinander getrennt
werden. Sie werden dann blockweise nebeneinander gesetzt. Zwischen den
einzelnen Signaturen wird jeweils \Macro{sigspace} eingefügt. Dieses ist mit
\Macro{quad} also einem horizontalen Abstand von einem Geviert voreingestellt.%
\EndIndexGroup
...
Anweisung entweder auf \Macro{centering} oder auf \Macro{raggedright} (nur
\Option{KOMAold}) gesetzt. Um innerhalb der Box eine linksbündige, zentrierte
oder rechtsbündige Ausrichtung zu erhalten, kann der Befehl mit
\Macro{renewcommand} zu \Macro{raggedleft}, \Macro{centering} oder
\Macro{raggedright} umdefiniert werden. Erfahrenen Anwender ist auch die
Verwendung von \Macro{let}
gestattet. Andere\ChangedAt{v3.44}{\Class{scrlttr2}\and \Package{scrletter}}
Definitionen sind nur zulässig, wenn gleichzeitig \Macro{sigalign} definiert
wird. In diesem Fall muss \Macro{sigalign} zu einer gültigen Spaltendefinition
expandieren, beispielsweise \texttt{l}, \texttt{c}, \texttt{r} oder auch
\texttt{p\{.5\Length{textwidth}\}}. \Macro{sigalign} ist maßgeblich für die
Ausrichtung innerhalb einer Signatur, falls diese mehrzeilig ist.
