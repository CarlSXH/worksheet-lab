...
For\ChangedAt{v3.44}{\Class{scrlttr2}\and \Package{scrletter}} letters with
multiple signatories, the individual signatures can be separated within the
\Variable{signature} variable by \Macro{and}. They are then placed as
blocks next to each others. Between the individual signatures,
\Macro{sigspace} is inserted. This is defined with \Macro{quad}, which is a
horizontal distance of 1\,em.% 
\EndIndexGroup
...
obtain flush-right, centered or flush-left alignment inside the box, you can
redefine the command using \Macro{renewcommand}. Experienced users are also
permitted to use \Macro{let}. Valid definitions are \Macro{raggedleft},
\Macro{centering} or
\Macro{raggedright}. Other\ChangedAt{v3.44}{\Class{scrlttr2}\and
  \Package{scrletter}} definitions are only permitted if \Macro{sigalign} is
defined at the same time. In this case, \Macro{sigalign} must expand to a
valid table column definition, for example \texttt{l}, \texttt{c}, \texttt{r}
or \texttt{p\{.5\Length{textwidth}\}}. \Macro{sigalign} is decisive for the
alignment within a signature if it is multi-line.
